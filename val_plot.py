...
metrics = sorted(metrics_50 + metrics_80 + metrics_90)
...
section_breaks = [3, 9, 15]  # Insert red line *before* these indices
for i in section_breaks:
    ax.axvline(x=i - 0.5, color='red', linestyle='--', linewidth=3)
# Final styling
ax.set_xticks(x)
ax.set_xticklabels(shortened_names, rotation=0, ha='center')
# ax.set_xticklabels(shortened_names, rotation=0), ha
ax.set_ylabel("Score")
ax.set_title("Model Comparison by Accuracy, F1, Recall: InceptionV3: 30 Epoch, MobileNetV3: 15 Epoch; Prune per layer")
ax.set_ylim(0, 1.05)
ax.legend()
plt.tight_layout()
