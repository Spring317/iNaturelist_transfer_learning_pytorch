...
def get_available_classes(dataset_path):
    ...
    # Remove the Insecta subdirectory assumption
...
def calculate_samples_per_class(class_counts, total_samples, class_indices):
    ...
    if total_count == 0:
        # If no sample counts, distribute evenly
        samples_per_class_equal = max(1, total_samples // len(subset_classes))
        return {cls["class_name"]: samples_per_class_equal for cls in subset_classes}
    ...
    for i, cls in enumerate(subset_classes):
        if i == len(subset_classes) - 1:  # Last class gets remaining samples
            samples = max(0, total_samples - allocated_samples)
        else:
            ratio = cls["sample_count"] / total_count
            samples = max(1, round(ratio * total_samples))
...
def create_test_set(
    dataset_path, class_counts, distribution_config, output_path, test_name
):
    ...
    evergestis_in_counts = any(
        cls["class_name"] == "Evergestis pallidata" for cls in class_counts
    )
    if evergestis_in_counts and "Evergestis pallidata" in missing_classes:
        print(
            "! Evergestis pallidata is in class_counts but missing from dataset directory"
        )
    ...
    for tier_name, (num_classes, tier_samples) in distribution_config.items():
        ...
        samples_per_class = calculate_samples_per_class(
            filtered_class_counts, tier_samples, class_indices
        )
        ...
        for class_idx in class_indices:
            ...
            # FIX: Ensure we don't try to sample more than available
            actual_samples = min(target_samples, len(available_images))

            if actual_samples <= 0:
                print(f"Warning: No samples to collect for class {class_name}")
                continue

            # Sample images safely
            if actual_samples == len(available_images):
                # If we need all images, just use them all
                selected_images = available_images.copy()
            else:
                # Only sample if we have more images than needed
                selected_images = random.sample(available_images, actual_samples)

            # Create class directory in output
            class_output_dir = os.path.join(test_output_path, class_name)
            ...
            for i, img_path in enumerate(selected_images):
                ...
                # Create new filename with class name and index
                filename = f"{class_name}_{i + 1:04d}{ext}"
            ...
            print(
                f"  {class_name}: {actual_samples}/{target_samples} samples (rank {class_info['rank']}, available: {len(available_images)})"
            )
...
def main():
    ...
    # Paths
    dataset_path = "/home/quydx/iNaturelist_transfer_learning_pytorch/test"
    class_counts_path = (
        "/home/quydx/iNaturelist_transfer_learning_pytorch/test_count_sorted.json"
    )
    output_path = "/home/quydx/iNaturelist_transfer_learning_pytorch/tests"
    ...
    if evergestis_in_counts:
        evergestis_info = next(
            cls for cls in class_counts if cls["class_name"] == "Evergestis pallidata"
        )
        print(
            f"✓ Evergestis pallidata found in class_counts at rank {evergestis_info['rank']} with {evergestis_info['sample_count']} samples"
        )
    else:
        print("✗ Evergestis pallidata NOT found in class_counts_sorted.json")

    # Check if class exists in dataset - Remove Insecta subdirectory
    evergestis_path = os.path.join(dataset_path, "Evergestis pallidata")
    ...
    for test_name, config in distributions.items():
        try:
            result = create_test_set(
                dataset_path, class_counts, config, output_path, test_name
            )
    ...
    for test_name, result in results.items():
        print(
            f"{test_name}: {result['total_samples']} samples, {result['total_classes']} classes"
        )
