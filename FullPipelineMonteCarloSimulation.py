    ...
    def _translate_prediction_to_global_label(self, prediction: int, model_type: ModelType):
        if model_type == ModelType.BIG_MODEL:
            ...
            if not global_species_labels:
                print(f"[Warning] Could not map species from big pred {big_species_labels} to global label")
            ...
            if not global_species_labels:
                print(f"[Warning] Could not map species from small pred {small_species_labels} to global label")
                return self.not_belong_to_global_idx
            return global_species_labels[0]
    ...
    def run(
        ...
        if save_path:
            ...
            with pd.option_context('display.max_rows', None, 'display.max_columns', None): 
                model_name = "pipeline_mbv3_90_convnext_full"
