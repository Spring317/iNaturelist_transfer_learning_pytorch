...
def evaluate_onnx_model(
    ...
    print(
        f"Model expects input shape: {input_shape}, using size: {img_height}x{img_width}x{channels}"
    )

    # Get class names using the same approach as before
    # datacreator = DatasetCreator(number_of_dominant_classes=7)
    # _, _, val_dataset, _, species_label = datacreator.create_dataset(0)
    # # Create dataloader
    ...
    _, val_loader, _, _, species_label = dataloader_creator.create_dataloader()
    ...
    print(
        f"Min/Max inference time: {np.min(inference_times):.2f}/{np.max(inference_times):.2f} ms"
    )
    ...
    # plt.text(
    #     avg_inference_time * 1.05,
    #     plt.ylim()[1] * 0.9,
    #     f"Mean: {avg_inference_time:.2f} ms",
    # )
    ...
    class_names = list(species_label.values())
    # Get detailed classification report
    report = classification_report(
        all_labels,
        all_preds,
        labels=list(range(len(species_label))),
        target_names=list(species_label.values()),
        output_dict=True,
        zero_division=0,
    )
    ...
    for i in range(num_classes):
        ...
        if np.sum(pred_class_i) > 0:
            ...
            conf_scores = np.array(
                [probs[i] for probs, pred in zip(all_probs, all_preds) if pred == i]
            )
...
def save_comprehensive_results(
    output_dir, all_preds, all_labels, all_probs, inference_times, class_names
):
    """
    ...
    inference_stats = {
        ...
        "iqr_ms": np.percentile(inference_times, 75)
        - np.percentile(inference_times, 25),
    }
    ...
    if len(errors) > 0:
        ...
        for i, row in errors.iterrows():
            ...
            true_prob = (
                row[f"prob_{class_names[true_label]}"]
                if true_label < len(class_names)
                else 0
            )
            pred_prob = (
                row[f"prob_{class_names[pred_label]}"]
        ...
        print(
            f"Saved {len(errors)} misclassifications to {output_dir}/misclassifications.csv"
        )
    ...
    for i, class_name in enumerate(class_names):
        ...
        if np.any(correct_indices):
            correct_probs = np.array(
                [p[i] for p, correct in zip(all_probs, correct_indices) if correct]
            )
        ...
        class_stats.append(
            {
                ...
                * true_positives
                ...
                "fraction_of_dataset": class_samples / total_samples
                if total_samples > 0
                else 0,
                "model_preference": class_predictions / total_samples
                if total_samples > 0
                else 0,
            }
        )
    ...
    print(
        f"Saved detailed per-class statistics to {output_dir}/detailed_class_metrics.csv"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model",
        type=str,
        default="models/mcunet_haute_garonne_8_species_quantized.onnx",
        help="Path to model file (.pth or .onnx)",
    )
    parser.add_argument(
        "--onnx",
        action="store_true",
        help="Evaluate ONNX model instead of PyTorch model",
    )
    parser.add_argument(
        "--dataset",
        type=str,
        default="val_dataset.pt",
