...
def eval_single(device, BATCH_SIZE, NUM_WORKERS, CLASS):
    ...
    with torch.no_grad():
        for images, labels in tqdm(val_loader, desc="Validating", unit="image"):
            ...
            # If class is "others" (e.g. CLASS - 1), route to Model 2
            if pred1 == CLASS - 1:
                # image_resized = F.interpolate(images, size=(224, 224), mode="bilinear", align_corners=False)
    ...
    weighted_recall = recall_score(all_labels, all_preds, average="macro")
    ...
    cm_df.to_csv(f"./confusion_matrix_{CLASS}.csv")
    ...
    pd.DataFrame([metrics]).to_csv(f"./metrics_{CLASS}.csv", index=False)
